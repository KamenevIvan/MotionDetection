...
class MOTMetricsCalculator:
    ...
    @staticmethod
    def match_objects(iou_matrix: np.ndarray, iou_threshold: float = 0.3) -> Dict[int, int]:
    ...
    def compute_tracking_metrics(self, gt: np.ndarray, det: np.ndarray) -> Dict[str, float]:
        ...
        for frame in sorted(all_frames):
            ...
            for gt_idx, det_idx in matches.items():
                ...
                if gt_obj_id in last_frame_seen:
                    if frame - last_frame_seen[gt_obj_id] > 1:
                        Frag += 1
                
                if gt_obj_id in prev_matches:
                    if prev_matches[gt_obj_id] != det_obj_id:
                        IDSW += 1
                        id_switch_details.append(
                            f"Frame {frame}: GT {gt_obj_id} switched from {prev_matches[gt_obj_id]} to {det_obj_id}"
                        )
                
                last_frame_seen[gt_obj_id] = frame
                TP += 1
        ...
        MOTA = 1 - (FN + FP + IDSW)/max(1, total_gt)
