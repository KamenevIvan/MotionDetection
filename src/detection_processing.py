...
def assignIDs(detections: list[list[Detection]], nf_threshold_id):
    ...
    for current_detection in detections[dcn]:
        ...
        for j, prev_detection in enumerate(framedatap):
            ...
            if (
                relSiou12 >= settings.iou_threshold_id
                and scalew12 <= settings.scale_threshold_id
                and scaleh12 <= settings.scale_threshold_id
            ):
    ...
    for idn in ids:
        ...
        for current_detection in detections[dcn]:
            if current_detection.id == idn:
                nf = (
                    current_detection.frames_count
                )  # number of frames the object with id has been detected
...
def validateObjs(
    detections: list[list[Detection]], frame_counter, fps, nf_threshold_id
):
        ...
        if current_detection.frames_count >= nf_threshold_id:
            ...
            if movement >= settings.tr_mov_threshold:
                if current_detection.last_frame == -1:
                    ...
                    current_detection.last_frame > -1
                    and (frame_counter - current_detection.last_frame)
                    % (settings.report_frame)
                    == 0
                ):
                    current_detection.for_report = True
                    nobsfr += 1
