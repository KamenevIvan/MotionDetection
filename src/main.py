...
def set_background_parameters(bs: cv.BackgroundSubtractorMOG2):
    bs.setBackgroundRatio(
        0.9
    )  # default 0.9, pixel constant for BgR * History is added to background model  ~ background update rate
    bs.setComplexityReductionThreshold(
        0.05
    )  # default 0.05 - ~number of samples for the component to be accepted to background
    bs.setVarThresholdGen(
        9.0
    )  # default 9.0, threshold of --||-- of sample to existing background component-above-start new component
    ...
    bs.setShadowThreshold(
        0.5
    )  # default 0.5, shadows darker than 0.5 are considered as moving objects
...
def main():
    ...
    bs = cv.createBackgroundSubtractorMOG2(
        history=settings.background_history, varThreshold=30, detectShadows=True
    )  # varThreshold - detection threshold
    ...
    while vcap.isOpened():
        ...
        if frame_counter > settings.frames_skip:
            ...
            contours, hierarchy = cv.findContours(
                foreground, cv.RETR_EXTERNAL, cv.CHAIN_APPROX_SIMPLE
            )
        ...
        detections, nidsc = detection_processing.completeIDs(
            detections, nf_threshold_id
        )
        ...
        detections, nobsfr = detection_processing.validateObjs(
            detections, frame_counter, fps, nf_threshold_id
        )
        detector_IO.print_console(
            f"{nobsfr} objects were marked suitable for reporting"
        )
        detector_IO.print_console(f"current framedata: {detections[len(detections)-1]}")

        detector_IO.print_frame_detection(
            settings.outputfile, detections[len(detections) - 1], frame_counter
        )
        frame_with_boxes = detector_IO.show_boxes(
            frame, detections[len(detections) - 1], fps
        )
        detector_IO.save_frame_with_boxes(
            frame_with_boxes,
            frame_counter,
            int(len(str(vcap.get(cv.CAP_PROP_FRAME_COUNT)))),
        )
    ...
    print(
        "Video processing stopped. Average time for frame processing (s):",
        ttime / frame_counter,
    )
    #############################################################
